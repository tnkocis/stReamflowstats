...
#
	...
	for (i in seq(1,length(years)-1,1)){
		by_year[[i]] <- filter(input$raw, (month >9 & year == years[i])| (month <10 & year == years[i]+1))
	...
	for (i in 1:length(by_year)){
		...
		stats[[i]][["Values"]]["Total_Q_acfte6"] <- sum(by_year[[i]][["Discharge_acfte6_day"]])
		stats[[i]][["Quantiles"]][["Quantiles_acfte6"]] <-quantile(by_year[[i]][["Discharge_acfte6_day"]], probs=c(0.05,0.1,0.2,0.25,0.5,0.75,0.9,0.95))
		stats[[i]][["Values"]]["Mean_acfte6"] <- mean(by_year[[i]][["Discharge_acfte6_day"]])
		stats[[i]][["Values"]]["Median_acfte6"] <- median(by_year[[i]][["Discharge_acfte6_day"]])
		stats[[i]][["Values"]]["min_acfte6"] <- min(by_year[[i]][["Discharge_acfte6_day"]])
		stats[[i]][["Values"]]["max_acfte6"] <- max(by_year[[i]][["Discharge_acfte6_day"]])
		names(stats)[i] <- names(by_year)[i]
	}
	

##### SEE CHAT WITH MICHAEL FROM HERE ON OUT ABOUT MISSING YEAR IN G3	
	summary <- list()
	summary[["total_Q_yearly"]] <- numeric(length(stats))
	for (i in 1: length(stats)){
		summary$total_Q_yearly[i] <- stats[[i]][["Values"]][["Total_Q_acfte6"]]
	}
	summary$mean_Q <- mean(summary$total_Q_yearly)
