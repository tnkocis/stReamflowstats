...
trendsfinal <- function(input, damyear, gauge, period){
	library(zoo)
	library(nlme)
	library(Kendall)
	...
	inputzero <- data.frame(sthyyear=input$sthyyear,zerolog=which(input$TotVolAbv_acft<=0))
	...
	for(i in 1:5){
		...
		if(length(metricraw<=5)){
			...
			metricraw_5 <- rollmean(metricraw_5,5)
			results_5$count_trend <- length(metricraw_5)
			lm_5 <- lm(metricraw ~ index(metricraw_5))
	...
	for(i in 1:5){
		...
		if(length(metricraw<=10)){
			...
			metricraw_10 <- rollmean(metricraw_10,10)
			results_10$count_trend <- length(metricraw_10)
			lm_10 <- lm(metricraw ~ index(metricraw_10))
	...
	if((sum(!yrseq%in%input$sthyyear, na.rm=TRUE)>5)|(which(rev(yrseq%in%input$sthyyear)==FALSE)<25)){
		...
		yrst <- rev(yrseq)[which(rev(yrseq%in%input$sthyyear)==FALSE)[[1]]]+1
		...
		results_5_frac$MKT_tau[[1]] <- MKT_5_frac$tau[[1]]
		results_5_frac$MKT_p[[1]] <- MKT_5_frac$sl[[1]]
		...
		results_10_frac$MKT_tau[[1]] <- MKT_10_frac$tau[[1]]
		results_10_frac$MKT_p[[1]] <- MKT_10_frac$sl[[1]]
