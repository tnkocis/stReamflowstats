...
r2testdays <- summary(lm(testdaysdf2$days~ testdaysdf2$year))$r.squared
r2testdays <- data.frame(r2=round(r2testdays,2))
testdaysplot2 <- ggplot(data=testdaysdf2, aes(x=year,y=days))+geom_line(size=1, color="blue")+
		...
		geom_smooth(method=lm, se=FALSE, color="red")+
		geom_text(data=r2testdays,aes(label=paste("R^2: ",r2, sep="")),parse=TRUE,x=2010,y=30)
ggsave(filename= "C:\\Users\\tiffn_000\\Desktop\\Figures\\WBpresentation\\daystrend_11402000.png", plot=testdaysplot2, dpi=300, height=8, width=10)

testnum <- rollmean(scatter_peakflowsdf3$`11402000`$pfstatsdf3$numpeaks,10,na.pad=TRUE)
testnumdf2 <- data.frame(days=testnum, year=scatter_peakflowsdf3$`11402000`$pfstatsdf3$year, yr1=scatter_peakflowsdf3$`11402000`$pfstatsdf3$numpeaks)
r2testnum <- summary(lm(testnumdf2$days~ testnumdf2$year))$r.squared
r2testnum <- data.frame(r2=round(r2testnum,2))
testnumplot2 <- ggplot(data=testnumdf2, aes(x=year,y=days))+geom_line(size=1, color="blue")+
		geom_line(aes(x=year,y=yr1))+
		xlab("Year") + ylab("Number of Peaks Above 90th Percentile")+
		ggtitle("Total Number of Peaks Above the 90th Percentile\n December to January")+
		geom_smooth(method=lm, se=FALSE, color="red")+
		geom_text(data=r2testnum,aes(label=paste("R^2: ",r2, sep="")),parse=TRUE,x=2010,y=9)
ggsave(filename= "C:\\Users\\tiffn_000\\Desktop\\Figures\\WBpresentation\\numpkstrend_11402000.png", plot=testnumplot2, dpi=300, height=8, width=10)

meanpks <- rollmean(scatter_peakflowsdf3$`11402000`$pfstatsdf3$mean_peakflow,10,na.pad=TRUE)
meanpksdf2 <- data.frame(days=meanpks, year=scatter_peakflowsdf3$`11402000`$pfstatsdf3$year, yr1=scatter_peakflowsdf3$`11402000`$pfstatsdf3$mean_peakflow)
r2meanpks <- summary(lm(meanpksdf2$days~ meanpksdf2$year))$r.squared
r2meanpks <- data.frame(r2=round(r2meanpks,2))
meanpksplot2 <- ggplot(data=meanpksdf2, aes(x=year,y=days))+geom_line(size=1, color="blue")+
		geom_line(aes(x=year,y=yr1))+
